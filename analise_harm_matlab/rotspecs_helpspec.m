...
% |                                                                  |
% |    Included new script rotspecs.m for rotary spectra             |
% |                                       (see reference j)          |
% |                                                                  |
%  ------------------------------------------------------------------
% |                                                                  
...
% | [hpos,hneg,htot,hrcoef,fff,conflim] = rotspecs(xxx1,xxx2,ppp,dt,win,smo,ci);
% |                                                                  
% | 1) INPUT / OUTPUT
...
% |    xxx1 = zonal      velocity for rotary spectra
% |    xxx2 = meridional velocity for rotary spectra
...
% |    hpos = positive rotation spectra for rotspecs
% |    hneg = negative rotation spectra for rotspecs
% |    htot = total spectra for rotspecs
% |    hrcoef = rotary coefficient for rotspecs
% |    fff    = frequencies
...
% |    
% | 3) SPECTRUM and CROSS SPECTRUM and ROTARY SPECTRUM
...
% |    Rotary spectra computed in the same way from total series,
% |        filtered afterwards
% | 
% | 4) TIME WINDOW CORRECTION
% |    Applied to spectrum and cross spectrum to correct for energy
% |    loss due to time window: (also for rotary spectrum
...
% |       semilogx(fff,hepya1.*fff*2.3) - energy conserving spectra
% | 
...
% |
% |    e) Rotary spectra
% |       loglog(fff,hpos,fff,hneg,fff,htot)
% |       loglog(fff,hrcoef)
% |       Para incluir amplitudes e eixos de rotacao, calcule
% |            apos = sqrt(hpos * ppp / dt); aneg = sqrt(hneg * ppp / dt);
% |            emax = apos + aneg; emin = abs(apos - aneg);
% |            loglog(fff,apos,fff,aneg)
% |            loglog(fff,emax,fff,emin)
% |
% |
% |
% |
