...
def curr_window(dep_sup, dep_bot, lon_e, lon_d, top_e, top_d, bot_e, bot_d, path,ticks, l_contour, cmap='cool_r', cbar_label='', cbar_ticks=[]):
    ...
    if l_contour[int(len(l_contour)/2-.5)] == 0:
        np.delete(l_contour, int(len(l_contour)/2-.5))
        cs0 = ax[0,0].contour(lon_e, -dep_sup, np.array(top_e).T, colors=contour_color,linewidths=1, levels=[0])
        cs0 = ax[0,1].contour(lon_e, -dep_sup, np.array(top_e).T, colors=contour_color,linewidths=1, levels=[0])
        cs0 = ax[1,1].contour(lon_e, -dep_sup, np.array(top_e).T, colors=contour_color,linewidths=1, levels=[0])
        cs0 = ax[1,0].contour(lon_e, -dep_sup, np.array(top_e).T, colors=contour_color,linewidths=1, levels=[0])


    if len(l_contour) == 2:
        cs0 = ax[0,0].contour(lon_e, -dep_sup, np.array(top_e).T, colors='white',linewidths=1, levels=[20])
        cs0 = ax[0,1].contour(lon_e, -dep_sup, np.array(top_e).T, colors='white',linewidths=1, levels=[20])
        cs0 = ax[1,1].contour(lon_e, -dep_sup, np.array(top_e).T, colors='white',linewidths=1, levels=[20])
        cs0 = ax[1,0].contour(lon_e, -dep_sup, np.array(top_e).T, colors='white',linewidths=1, levels=[20])

        cs0 = ax[0,0].contour(lon_e, -dep_sup, np.array(top_e).T, colors='black',linewidths=1, levels=[50])
        cs0 = ax[0,1].contour(lon_e, -dep_sup, np.array(top_e).T, colors='black',linewidths=1, levels=[50])
        cs0 = ax[1,1].contour(lon_e, -dep_sup, np.array(top_e).T, colors='black',linewidths=1, levels=[50])
        cs0 = ax[1,0].contour(lon_e, -dep_sup, np.array(top_e).T, colors='black',linewidths=1, levels=[50])
...
for section in sections:
    ...
    contours = {
        1 : np.round(np.arange(-.3, .35, .05), 2),
        2 : np.round(np.arange(-.3, .35, .05), 2),
        3 : np.round(np.arange(-.3, .35, .05), 2),
        4 : np.round(np.arange(-.25, .3, .05), 2),
        5 : np.round(np.arange(-.15, .16, .05), 2),
        6 :  np.round(np.arange(-.5, .55, .05), 2), 
        7 : np.round(np.arange(-.1, .12, .05), 2)
    }
    ...
    curr_window(reanal_subset['depth'].sel(depth=slice(0,div_d)), reanal_subset['depth'].sel(depth=slice(div_d,10000)),
                    dist[:m_lons+1], dist[m_lons:],
                        ...
                        np.arange(-vals_mean_max, vals_mean_max + (vals_mean_max)/1000,(vals_mean_max)/1000), l_contour=l_contour, cmap='bwr',
                        cbar_label='(m/s)', cbar_ticks=cbar_ticks)
    ...
    curr_window(reanal_subset['depth'].sel(depth=slice(0,div_d)), reanal_subset['depth'].sel(depth=slice(div_d,10000)),
                    ...
                    np.arange(0,vals_var_max, vals_var_max/1000), l_contour=l_contour, cmap = 'inferno', cbar_label='(m/s)\u00B2')
    ...
    curr_window(reanal_subset['depth'].sel(depth=slice(0,div_d)), reanal_subset['depth'].sel(depth=slice(div_d,10000)),
                    ...
                    output_dir + f'/{s}/std',
                    np.arange(0,vals_std_max, vals_std_max/1000), l_contour=l_contour,
    ...
    contours = {
        1 : np.round(np.arange(0, .3, .03), 2),
        2 : np.round(np.arange(0, .3, .03), 2),
        3 : np.round(np.arange(0, .3, .03), 2),
        4 : np.round(np.arange(0, .3, .03), 2),
        5 : np.round(np.arange(0, .3, .03), 2),
        6 :  np.round(np.arange(0, .3, .03), 1), 
        7 : np.round(np.arange(0, .3, .03), 2)
    }
    ...
    curr_window(reanal_subset['depth'].sel(depth=slice(0,div_d)), reanal_subset['depth'].sel(depth=slice(div_d,10000)),
                    ...
                    np.sqrt(varf_bot_e), np.sqrt(varf_bot_d),
                    output_dir + f'/{s}/stdf',
                    np.arange(0,vals_stdf_max, vals_stdf_max/1000), l_contour=l_contour,
                    cbar_ticks=cbar_ticks, cmap='inferno', cbar_label='(m/s)')
