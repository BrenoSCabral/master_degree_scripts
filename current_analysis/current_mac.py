...
def curr_window(dep_sup, dep_bot, lon_e, lon_d, top_e, top_d, bot_e, bot_d, path,ticks, l_contour, cmap='cool_r', cbar_label='', cbar_ticks=[]):
    # aumentar numero de ticks pdoe dar resultado melhor, mas preciso tbm testar com o
    # imshow. Verificar tbm a estrutura de profundidade. Me parece
    # que o problema ta surgindo la.
    ...
    if l_contour[int(len(l_contour)/2-.5)] == 0:
        ...
        ax[0,0].contour(lon_e, -dep_sup, np.array(top_e).T, colors=contour_color ,linewidths=1, levels=np.asarray([0]))
        ax[0,1].contour(lon_d, -dep_sup, np.array(top_d).T, colors=contour_color ,linewidths=1, levels=np.asarray([0]))
        ax[1,1].contour(lon_d, -dep_bot, np.array(bot_d).T, colors=contour_color ,linewidths=1, levels=np.asarray([0]))
...
for section in sections:
    ...
    for lat, lon in zip(lats, lons):
        ...
        if type(val_sec) !=  np.ndarray:
            val_sec = mean_top.sel(latitude=lat, longitude=lon, method='nearest').fillna(0).values
        elif (mean_top.sel(latitude=lat, longitude=lon, method='nearest').fillna(0).values != val_sec).sum() == 0:
                continue
        else:
            val_sec = mean_top.sel(latitude=lat, longitude=lon, method='nearest').fillna(0).values


        #### IPC::: ALTERACAO PRA CONSERTAR O PERFIL
        ### ALTERACAO NOVA
        #### BRENO ALTERACAO
        if lon < lons[m_lons]:
    ...
    output_dir = f'/Users/breno/mestrado/curr_0720/'
    ...
    curr_window(reanal_subset['depth'].sel(depth=slice(0,div_d)), reanal_subset['depth'].sel(depth=slice(div_d,10000)),
                    dist[:m_lons+1], dist[m_lons:],
                        mean_top_e, mean_top_d, mean_bot_e, mean_bot_d, output_dir + f'/mean_{s}',
                        ticks[s], l_contour=l_contour, cmap='bwr',
                        cbar_label='(m/s)', cbar_ticks=cbar_ticks)

##---> rever essa funcao
# n_arr = []
# vals = []
# for d in  n_mean_top_e[0].depth:
#     for i in n_mean_top_e:
#         vals.append(i.sel(depth=d))
#     n_arr.append(vals)
#     vals = []
    ...
    contours = {
        ...
        7 : np.round(np.arange(0, .31, .03), 2)
    }
    ...
    curr_window(reanal_subset['depth'].sel(depth=slice(0,div_d)), reanal_subset['depth'].sel(depth=slice(div_d,10000)),
                    dist[:m_lons+1], dist[m_lons:],
                    perc_top_e, perc_top_d, perc_bot_e, perc_bot_d, output_dir + f'/perc_{s}', np.arange(0,100.1,.1),
                    cbar_ticks=cbar_ticks, l_contour=np.array([20, 50]), cmap='inferno', cbar_label='%')
